...
| `relaxation_vs_optimal.ipynb` | Compares the efficient approach with the fully optimal LP  on a 1D chain of length $L = 10$. Plots the gap $\Delta\|\boldsymbol{\lambda}\|_1$ as a function of $s/r$. | `relax_vs_optimal.pdf` |
| `tradeoff_graphs.ipynb` | Scaling of the mean quantum runtime $\langle\|\boldsymbol{\lambda}\|_1\rangle$ with the number of fermionic modes $n$ (10–40) for fixed ratios $s/r \in \{6,7,8,9\}$ on a 1D chain. | `trade_off.pdf` |
